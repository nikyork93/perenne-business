...
function renderEmail(p: RenderEmailParams): string {
  return `<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><title>${escapeHtml(p.title)}</title></head>
<body style="margin:0;padding:0;background:#0a0a0f;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;color:#f4f4f5;">
  ...
  <!-- ambient glow background, simulated with table layered backgrounds -->
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#0a0a0f;background-image:radial-gradient(ellipse at 20% 10%, rgba(74,122,140,0.18) 0%, transparent 50%),radial-gradient(ellipse at 80% 90%, rgba(44,88,104,0.15) 0%, transparent 50%);padding:48px 20px;">
    <tr><td align="center">

      <!-- liquid glass card -->
      <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:rgba(24,24,27,0.85);border:1px solid rgba(255,255,255,0.08);border-radius:24px;overflow:hidden;box-shadow:0 20px 60px rgba(0,0,0,0.4);">
        ...
        <tr><td align="center" style="padding:0 40px 8px;">
          <table role="presentation" cellpadding="0" cellspacing="0" border="0">
            <tr><td align="center" bgcolor="#4a7a8c" style="border-radius:14px;">
              <a href="${p.ctaUrl}" style="display:inline-block;background:#4a7a8c;color:#ffffff;text-decoration:none;font-size:15px;font-weight:500;padding:14px 36px;border-radius:14px;letter-spacing:0.01em;border:1px solid rgba(255,255,255,0.12);">${escapeHtml(p.ctaText)}</a>
            </td></tr>
          </table>
        </td></tr>
        ...
        <!-- separator + brand -->
        <tr><td style="padding:18px 40px;border-top:1px solid rgba(255,255,255,0.06);background:rgba(0,0,0,0.2);">
